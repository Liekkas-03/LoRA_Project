"""Generate math answers with a base model and a LoRA adapter on cloud GPU.

本文件用于在 AutoDL 云端加载 base model + LoRA adapter，对 dev/test JSONL
生成 prediction 字段。它延迟导入 PyTorch/Transformers/PEFT，避免本地轻量环境安装重依赖。
"""
...
def read_jsonl(path: Path, limit: int | None = None) -> list[dict[str, Any]]:
    # 读取待推理样本；limit 用于 smoke run 时只取前若干条快速验证。
...
def write_jsonl(path: Path, records: list[dict[str, Any]]) -> None:
    # 将带 prediction 的样本写回 JSONL，供 evaluate_accuracy.py 继续评测。
...
def build_prompt(record: dict[str, Any]) -> str:
    # 推理时不能泄露标准解答，只给题目，并让模型从 Solution: 后续写。
...
def generate_records(
    ...
    # 批量遍历样本生成 prediction；保留原字段，便于后续按数据集/难度分组评测。
...
def main() -> None:
    # 命令行入口：云端传入 base model、adapter、输入和输出路径即可生成预测文件。
